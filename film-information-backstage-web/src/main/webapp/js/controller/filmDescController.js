 //控制层 
app.controller('filmDescController' ,function($scope,$controller   ,filmDescService,uploadService){
	...
	$scope.uploadImage=function () {
		uploadService.uploadFile().success(
			function (response) {
				if(response.error==0){
					//成功
					$scope.entity.filmImage =response.url;
				}else{
					alert("md，就不信了："+response.message);
				}
			}).error(
			function (){
				alert("上传出错")
